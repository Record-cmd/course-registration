...
export default {
    ...
    methods:{
        cancel(LectureId){
        this.items = this.items.filter((v) => v.lectureId !== LectureId);
        //console.log(LectureId);//input디버깅 확인용
        this.$socket.emit('DeleteCheck', LectureId)
      },
      ...mapActions(useCounterStore,['CancelLecture'])
    },
      ...
      <v-row>
        ...
        >
          <v-card>
            ...
            <v-divider></v-divider>
            <v-btn @click = "cancel(item.raw.lectureId),CancelLecture()">
            취소
            </v-btn>
          </v-card>
        </v-col>
